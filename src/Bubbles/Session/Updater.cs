...
public sealed class Updater : IDisposable
{
    ...
    internal static string? ParseChecksum(string listing, string name)
    {
        foreach (var line in listing.Split('\n'))
        {
            var parts = line.Trim().Split((char[]?)null, StringSplitOptions.RemoveEmptyEntries);
            // TrimStart('*') because GNU sha256sum marks binary mode that way. Our own
            // workflow does not, but a listing regenerated by hand otherwise silently fails
            // to match and the update is refused with no obvious reason.
            if (parts.Length >= 2 &&
                string.Equals(Path.GetFileName(parts[^1].TrimStart('*')), name, StringComparison.OrdinalIgnoreCase))
            {
